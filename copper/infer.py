...
def infer(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    model.eval()
    image_path = cfg.get('image_path')  # Replace with the path to your image
    image = preprocess_image(image_path)
    # for predictions use trainer.predict(...)
    #predictions = trainer.predict(model=model, datamodule=datamodule, ckpt_path='/Users/mdhakad/Desktop/emlo4-lightning/outputs/2023-07-02/22-57-40/lightning_logs/version_0/checkpoints/epoch=0-step=313.ckpt')
    with torch.no_grad():
        output = model(image)
